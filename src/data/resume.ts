...
export const experiences: Experience[] = [
    ...
    projects: [
      {
        ...
        points: [
          '웹(카카오·휴대폰)과 웹뷰 분기 로그인·회원가입 플로우를 구축하고 미들웨어 기반 라우트 가드와 웹뷰 브릿지 통신을 통해 앱-웹 간 인증 상태를 동기화했습니다.',
          'safe-area 대응, 플랫폼별 이벤트 분기, 네이티브 브릿지 메시지 공통화를 적용하여 하나의 코드베이스로 웹·웹뷰 두 플랫폼을 안정적으로 지원하도록 했습니다.',
          '상태 관리 라이브러리 마이그레이션을 통해 클라이언트·서버 상태를 분리하고 API 공개/인증 쿼리 분기 구조 설계, 타입 체계 재편, 공통 컴포넌트 설계 등 코드베이스 전반의 구조를 개선했습니다.',
          '지도 롱프레스·조과 상세 기반 두 가지 북마크 저장 플로우를 설계하고 좌표 기반 주소 자동 변환, 이미지 업로드, 폴더 관리 등 부가 기능을 구현했습니다.',
          '조과 상세 페이지를 각 섹션으로 컴포넌트화하고 댓글 시스템(작성·수정·삭제·답글·무한스크롤)을 신규 개발하여 캐시 최적화로 실시간 반영 성능을 확보했습니다.',
        ],
      },
      {
        title: '예약 파트너 관리 웹앱 신규 구축',
        points: [
          '파트너사가 직접 상품·예약을 관리하는 운영 웹앱을 신규로 설계하고 구축했습니다.',
          '홈 예약 현황, 예약 캘린더, 상품 관리 화면을 중심으로 선박·낚시터 상품 등록·수정·일괄 처리, 직접 예약 플로우를 구현했습니다.',
          'React-Query 기반 API 구조를 도메인별로 정리하고 날짜·예약 상태 동기화 이슈를 개선해 운영 안정성을 높였습니다.',
        ],
      },
    ],
  },
    ...
    projects: [
      {
        ...
        points: [
          '검색 결과 페이지(브랜드·디렉터·앰버서더·기획전) UI를 개발하고 Amplitude 기반 클릭 이벤트 수집 로직을 설계하여 배포 후 데이터 분석이 가능하도록 준비했습니다.',
          '카테고리 미노출 이슈를 분석하고 백엔드 팀과 협업하여 새로운 API 명세를 설계했습니다. 카테고리 분리 로직 및 성별 기반 필터링을 구현하여 중복 노출 문제를 해결했습니다.',
          '어드민 일본어 번역 누락 개선, 배송 기간 우선순위 노출 로직 개선 등 다양한 운영 이슈를 신속하게 대응했습니다.',
        ],
      },
    ],
  },
    ...
    projects: [
      {
        ...
        points: [
          '서버 기반 마커 클러스터링을 클라이언트 기반 네이버 맵 마커 클러스터링으로 전환하여 개별 조과 데이터를 직접 활용할 수 있는 구조로 개선하고 조과 상세 동적 라우트, robots.txt, 사이트맵, 메타 태그를 적용해 SEO 구조를 구축했습니다.',
          '구글 서치콘솔 기준 CTR 11.3%, 클릭 수 +41.8% / 네이버 서치어드바이저 기준 활성 사용자 +61.1%, 이벤트 수 +50.8% 달성했습니다.',
          '검색 유입 기반 사용자 흐름을 분석해 어바웃피싱 샵 상품을 노출·연계하여 매출 증대에 기여하고 예약 마커·이벤트 배너 구현으로 예약 플랫폼 초기 유입을 확보했습니다.',
        ],
      },
      {
        title: '중고장터',
        points: [
          '이니시스 인증 API 연동을 통한 본인 확인, 팝빌 API 연동을 통한 계좌 실명 인증, 더치트 사기이력조회 API 연동으로 중고거래 서비스의 안전성과 신뢰성을 강화했습니다.',
          '키워드 알림, 리뷰 관리, 신고 내역, 인증센터 등 마이페이지 기능 전반을 개발했습니다.',
          '중고장터 백오피스에서 판매글 관리, 유저 신고, 거래 상태 확인, 상품 등록 및 검수 시스템을 개발하여 운영 효율성을 높였습니다. 홈 모듈 관리 기능을 개발하여 판매글·배너 등 노출 컴포넌트의 등록·기간·노출 여부를 관리자가 직접 제어할 수 있도록 구성했습니다.',
        ],
      },
      {
        title: '백오피스',
        points: [
          '커뮤니티 관리 기능 고도화(신규 모임 생성·초대·추천 모임 설정)와 회원 탈퇴 사유 조회, 유저 관심사 기반 맞춤형 홈 모듈 관리 기능을 구현하여 운영 효율성과 개인화 콘텐츠 제공에 기여했습니다.',
        ],
      },
      {
        title: '어바웃피싱 샵',
        points: [
          '장바구니 기능(상품 선택·수량 조절·쿠폰 적용)을 개발하고 결제 금액 계산 오류, 쿠폰 표시 조건 등 다양한 예외 상황을 반영하여 서비스 품질을 향상시켰습니다.',
        ],
      },
      {
        title: '낚시대회 고객사 관리자 시스템',
        points: [
          '홍보 문의, 모집 대행, 대회 관리(참여자 현황·기록 등록·순위) 기능을 구현하여 대회 운영 효율성 향상 및 플랫폼 서비스 자동화에 기여했습니다.',
        ],
      },
    ],
  },
    ...
    projects: [
      {
        ...
        points: [
          'React, Redux, Next.js, SWR, Vercel 기반으로 서비스를 재개발하고 장바구니 및 주문 플로우 페이지의 프론트엔드 개발을 담당했습니다.',
          '상품별 관부가세, 프로모션, 포인트 등 복잡한 할인 정책 계산 로직을 구현하고 외부 PG 연동(토스페이, 카카오페이)으로 전환하여 CVR 14.27% 증가를 달성했습니다.',
          '주문/배송 조회, 취소/반품 처리 등 주문 관리 기능과 어드민 기능을 개발했습니다.',
  ...
  },
  {
    ...
    projects: [
      {
        title: '스마트피싱보호',
        points: ['Vue2에서 React로 마이그레이션하며 홈페이지를 리뉴얼하고 부가 서비스 웹뷰 콘텐츠 페이지를 개편했습니다.'],
      },
      {
        title: '휴대폰가족보호',
